...
export default function MeditationTimerWithAmbient() {
  ...
  const soundOptions = useMemo(() => [
    ...
    { id: 'rain', name: 'Pluie douce', icon: '🌧️', file: '/assets/ambient/rain.mp3', type: "audio/mpeg",}
  ], []);
  ...
  // Ajoutez un useEffect pour déboguer
  useEffect(() => {
    console.log('Sound Options:', soundOptions);
    console.log('Selected Sound:', selectedSound);
    console.log('Selected Sound Option:', selectedSoundOption);
  }, [selectedSound, selectedSoundOption, soundOptions]);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Élément audio caché - TOUJOURS dans le DOM */}
      ...
      {!isPlaying && !isFinished && (
        <div style={{
          ...
          <p style={{ fontSize: '1rem', fontWeight: '600', color: '#1f2937' }}>
            {duration} min • {selectedSoundOption?.icon} {selectedSoundOption?.name}
          </p>
        </div>
      )}
    </div>
